...
const resultPlace :Element | null = document.querySelector('#result')
...
btn.addEventListener('click', (e) => {
    ...
    while (resultPlace?.children.length > 0) {
        resultPlace?.children[0].remove();
    }
...
let CopyFunc = () => {
    const resultPlace: Element | null = document.querySelector('#result');
    if (resultPlace?.children.length === 0) {
        alert('there is nothing to copy!');
    } else {
        // @ts-ignore
        if ("children" in resultPlace) {
            navigator.clipboard.writeText(<string>resultPlace.children[0].textContent)
                .then(() => alert("Copied!"))
